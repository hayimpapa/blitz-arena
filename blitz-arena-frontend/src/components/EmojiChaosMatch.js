...
export default function EmojiChaosMatch({ userId, username, onBackToLobby }) {
  ...
  useEffect(() => {
    ...
    return () => {
      socket.emit('leave_queue', { gameType: 'emojiChaosMatch' });
  ...
  }, [socket, userId, username, onBackToLobby, playerNumber]);
  ...
  const handleCardClick = (position) => {
    if (gameState !== 'playing') return;
    if (currentPlayer !== playerNumber) return;
    if (flippedCards.includes(position)) return;
    if (matchedCards.includes(position)) return;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-pink-600 to-purple-600 p-4 relative">
      ...
      <div className="max-w-2xl mx-auto pt-8">
        <div className="bg-white rounded-3xl shadow-2xl p-6 mb-6">
          <div className="flex justify-between items-center mb-4">
            <div className="text-center flex-1">
              ...
              <div className="text-2xl font-bold text-pink-600">{myScore}</div>
              ...
              <button
                ...
                className={`aspect-square rounded-2xl text-5xl font-bold transition-all ${
                  matchedCards.includes(index)
                    ...
                    : 'bg-gradient-to-br from-pink-400 to-purple-500 hover:from-pink-500 hover:to-purple-600 active:scale-95'
        ...
        {gameState === 'match_end' && (
          <div className="text-center space-y-4">
            <button
              onClick={handleBackToLobby}
              className="bg-white text-pink-600 font-bold py-4 px-8 rounded-xl hover:bg-gray-100 transition-colors"
            >
              Back to Lobby
            </button>
          </div>
